...
for (let r = 0; r < rows.length; r++) {
  ...
  let cols = row.split("");
  grid.push(cols);
  let endCols = row.replace(/./g, ".");
  endGrid.push(endCols.split(""));
...
endGrid[node.r][node.c] = node.v;
let originalDirection = node.d;
while (!finishedLoop) {
  if (node.v === "S") {
    finishedLoop = true;
    if (originalDirection === 0 && node.d === 0) {
      endGrid[node.r][node.c] = "|";
    } else if (originalDirection === 1 && node.d === 1) {
      endGrid[node.r][node.c] = "-";
    } else if (originalDirection === 0 && node.d === 3) {
      endGrid[node.r][node.c] = "L";
    } else if (originalDirection === 0 && node.d === 1) {
      endGrid[node.r][node.c] = "J";
    } else if (originalDirection === 2 && node.d === 1) {
      endGrid[node.r][node.c] = "7";
    } else if (originalDirection === 1 && node.d === 0) {
      endGrid[node.r][node.c] = "F";
    }
  } else {
    ...
    endGrid[newNode.r][newNode.c] = newNode.v;
    node = newNode;
    steps++;
  }
}

for (let r = 0; r < endGrid.length; r++) {
  let insideLoop = false;
  for (let c = 0; c < endGrid[r].length; c++) {
    let value = endGrid[r][c];
    if (value === "|" || value === "L" || value === "J") {
      insideLoop = !insideLoop;
    } else if (insideLoop && value === ".") {
      total++;
      endGrid[r][c] = "O";
    }
  }
