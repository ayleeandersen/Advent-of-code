...
for (let i = 0; i < input_array.length; i++) {
  ...
  for (let j = differences.length - 1; j >= 1; j--) {
    let lastDifference = differences[j][differences[j].length - 1];
    let nextLastDifference = differences[j-1][differences[j-1].length - 1];
    differences[j-1].push(nextLastDifference + lastDifference);
  }
  let nextNum = differences[0][differences[0].length - 1];
